...
export default async function HomePage() {
  ...
  return <main>
    <section className="shell lead-section">
      {lead ? <div className="lead-story"><div className="lead-copy">{(lead.categories?.[0]?.name || (lead.is_breaking ? "Breaking news" : "")) && <p className="lead-category">{lead.categories?.[0]?.name || "Breaking news"}</p>}<h1><Link href={`/posts/${lead.slug}`}>{lead.title}</Link></h1>{lead.dek && <p className="lead-dek">{lead.dek}</p>}<div className="lead-footer"><Link className="arrow-link" href={`/posts/${lead.slug}`}>Read the story <span>→</span></Link><span>{lead.author_name ?? "Revile newsroom"}</span></div></div>{lead.featured_image_url ? <Link href={`/posts/${lead.slug}`} className="lead-image"><img src={lead.featured_image_url} alt={lead.featured_image_alt} /></Link> : <div className="lead-art" aria-hidden="true"><span>R</span></div>}</div> : <div className="empty-state"><h1>The desk is quiet.</h1><p>No published stories are available yet.</p></div>}
    </section>
