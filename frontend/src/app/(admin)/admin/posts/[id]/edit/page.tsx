...
export default function EditPostPage() {
  ...
  async function submit(values: PostFormValues) {
    ...
    setError("");
    try { await api.updatePost(post.id, { ...values, dek: values.dek || null }, accessToken); router.push("/admin/posts"); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to update post."); }
  }
  ...
  return <main className="admin-content narrow-content"><p className="eyebrow">Edit dispatch</p><h1>Refine the story.</h1><p className="muted">{post.status} · Last updated {new Date(post.updated_at).toLocaleString()}</p><PostForm initialValues={{ title: post.title, slug: post.slug, dek: post.dek || "", body: post.body, is_breaking: post.is_breaking }} onSubmit={submit} submitLabel="Save changes" submittingLabel="Saving..." error={error} /></main>;
}
