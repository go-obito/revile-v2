  ...
  return (
    <article className={compact ? "post-card post-card-compact" : "post-card"}>
      ...
      <div className="post-card-content">
        <div className="post-card-meta"><span>{section}</span><time dateTime={post.published_at ?? post.created_at}>{new Date(post.published_at ?? post.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</time></div>
